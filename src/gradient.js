...
gradient.colors = [[0, 0, 255], [255, 0, 0]]

gradient.nameToHex = function (colour) {
    const colours = {
        ...
        "pink": "#ffc0cb", "plum": "#dda0dd", "powderblue": "#b0e0e6", "purple": "#800080", "rebeccapurple": "#663399", "red": "#ff0000",
...
gradient.hexToRGB = function (color) {

    let c = gradient.nameToHex(color)
...
gradient.RGBToHex = function (color) {

    let hex = "#"
    hex += ("0" + color[0].toString(16)).slice(-2)
    hex += ("0" + color[1].toString(16)).slice(-2)
    hex += ("0" + color[2].toString(16)).slice(-2)
    return hex
}

gradient.set = function (colorsHex) {

    if (colorsHex && colorsHex.length > 1) {

        gradient.colors = colorsHex.map(function (colorHex) {
            return gradient.hexToRGB(colorHex)
        })
    }
}

gradient.at = function (value) {

    const getIndex = function (index, min, max) {
        return index < min ? min : index > max ? max : index;
    }

    const len = gradient.colors.length - 1
    const delta = 1 / len
    const dValue = value >= 1 ? 1 : (value % delta) / delta
    const index = Math.floor(value / delta)
    const colorFrom = gradient.colors[getIndex(index, 0, len - 1)]
    const colorTo = gradient.colors[getIndex(index + 1, 1, len)]

    let c = [0, 0, 0]
    c[0] = Math.floor(colorFrom[0] * (1 - dValue) + dValue * colorTo[0]);
    c[1] = Math.floor(colorFrom[1] * (1 - dValue) + dValue * colorTo[1]);
    c[2] = Math.floor(colorFrom[2] * (1 - dValue) + dValue * colorTo[2]);

    return gradient.RGBToHex(c);
}
